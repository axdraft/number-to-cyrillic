...
function numberToString(number, options) {
   ...
   */
  var defaultOptions = {
    onlyNumber  : (options && options.onlyNumber) || false,
    withCurrency: (options && options.currency) || true,
    showDecimal : (options && options.showDecimal) || false
  };
  /**
   ...
   */
  if (!defaultOptions.withCurrency) {
    currencyValues = {
  ...
  /**
   * Whether onlyNumber is ON - don't show fractional part and additional strings
   */
  if (defaultOptions.onlyNumber) {
    currencyValues = {
      currencyNameCases: ['', '', ''],
      decimalNameCases : ['', '', ''],
      currencyValueCase: 1,
   ...
   */
  for (
    var currentUnitClass = classesOfUnits;
    currentUnitClass > 0;
    currentUnitClass--
  ) {
    // Digit case
    ...
    for (; currentClassNumber.length > 0;) {
      // Current digit
      var currentDigit = parseInt(
        currentClassNumber.substr(0, 1)
      );
      ...
      if (currentUnitClass === 1) {
        if ((currentClassNumber.length !== 2 || currentDigit !== 1) && currentClassNumber < 3) {
    ...
    currentClassNumber = integerValue
      .toString()
  ...
  for (
    ...
    // Current digit
    var currentFractinalDigit = parseInt(
      fractionalValue[fractionalIndex]
    );

    // If the digit in the range 10-19
    if (
      parseInt(fractionalValue) > 9 &&
      parseInt(fractionalValue) < 20
    ) {
      fractionalWords.push(
        textValues.ua.tens[fractionalValue.substr(1, 1)]
      );
      fractionalIndex = 2;
      ...
      if (fractionalIndex === 0) {
        ...
        fractionalWords.push(
          textValues.ua.numbers[currencyValues.decimalValueCase][currentFractinalDigit]
        );
      }
  ...
  // If we should add decimal part
  if (defaultOptions.showDecimal) {
