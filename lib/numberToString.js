...
function numberToString(money) {
  ...
  return isNegative ? 'мінус ' : '' + result.mixed.substr(0, 1).toUpperCase() + result.mixed.substr(1);
}
  ...
  if (price) {
    for (i = 0; i < price.length; i += 3) {
      ...
      if (_n(price, i + 2, 2) > 10 && _n(price, i + 2, 2) < 20) {
        ...
        (unit === 'один' && (i === 3 || D === centToWordArray)) ? unit = 'одна' : 0;
        (unit === 'два' && (i === 3 || D === centToWordArray)) ? unit = 'двi' : 0;
        i === 0 && unit != '' ? 0 : unit += ' ' + matrix[_n(price, i + 1, 1)][i / 3 + 3];
      ...
      if (price.substr(price.length - i - 3, 3) === '000' && unit === ' ' + matrix[0][i / 3 + 3]) unit = '';
