...
function numberToString(number, options) {
  ...
  if (options) {
    ...
    if (typeof options.currentCurrency === 'string') {
      defaultOptions.currentCurrency = options.currentCurrency;
    }
  }
   ...
   */
  var currencyValues = textValues.ua.currency[defaultOptions.currentCurrency];
      ...
      if (currentClassNumber.length === 2 && currentDigit === 1) {
        ...
        if (currentUnitClass === 2 && currentClassNumber.length === 1) {
          ...
          if (currentClassNumber.length > 1) {
            ...
            if (currentDigit === 1 && defaultOptions.currentCurrency === 'uah') {
              result.push(textValues.ua.numbers[0][currentDigit]);
            } else if (currentDigit === 2 && defaultOptions.currentCurrency === 'uah') {
              result.push(textValues.ua.numbers[0][currentDigit]);
            } else {
  ...
  for (fractionalIndex = 0; fractionalIndex < fractionalValue.length; fractionalIndex++) {
    ...
    if (parseInt(fractionalValue) > 9 && parseInt(fractionalValue) < 20) {
      ...
      if (fractionalIndex === 0) {
        ...
        fractionalWords.push(textValues.ua.numbers[currencyValues[defaultOptions.currentCurrency].decimalValueCase][currentFractinalDigit]
        );
      }
  ...
  return {
    value: finalResult,
    currency: currencyValues.currencyNameCases[currencyTextCase]
  };
}
